...
public final class ErlangCoverageSensor extends AbstractErlangSensor {
	...
	protected void analyseCoveredFile(Project project, SensorContext sensorContext, CoverFileCoverage coveredFile,
		...
		if (coveredFile.getCoveredLines() > 0) {
			 ...
			 * else {
			 * 
			 * // colour all lines as not executed for (int x = 1; x <
			 * sensorContext.getMeasure(sourceResource,
			 * CoreMetrics.LINES).getIntValue(); x++) { lineHitsData.add(x, 0); }
			 * 
			 * // use non comment lines of code for coverage calculation Measure
			 * ncloc = sensorContext.getMeasure(sourceResource, CoreMetrics.NCLOC);
			 * sensorContext.saveMeasure(sourceResource, lineHitsData.build());
			 * sensorContext.saveMeasure(sourceResource,
			 * CoreMetrics.LINES_TO_COVER, ncloc.getValue());
			 * sensorContext.saveMeasure(sourceResource,
